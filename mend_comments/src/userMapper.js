// FORKED FROM jira/jira-data/sync_issue_parents/src/userMapper.js @ 003579d
// Changes:
//   - added resolveByEmail(email) — exact-email-match only (no fallback to
//     displayName or username-prefix heuristics). This is the only resolver
//     mend_comments uses, per the explicit project rule that DC username →
//     DC email → Cloud accountId is the canonical path.
...
class UserMapper {
  ...
  /**
   * Email-exact resolver. Searches Cloud /user/search?query=<email> and picks
   * the candidate whose `emailAddress` field equals the supplied email
   * (case-insensitive). No fallback to displayName or username-prefix matching.
   *
   * Returns the same shape as resolve():
   *   { accountId, source, note, candidateCount, displayName, emailAddress }
   *
   *   source ∈ "cache" | "email-exact" | null
   *   note   ∈ null | "no_match" | "no_exact_email_match" | "ambiguous_exact_email_N" | "lookup_error: ..."
   */
  async resolveByEmail(email) {
    ...
    if (!trimmedEmail) {
      return {
        accountId: null,
        source: null,
        note: "lookup_error: no email supplied",
        candidateCount: 0,
        displayName: null,
        emailAddress: null,
      };
    ...
    const cacheKey = `email||${lowerEmail}`;
    if (this.cache[cacheKey]) {
      const cached = this.cache[cacheKey];
      this.stats.cacheHits++;
      if (cached.accountId) this.stats.resolved++;
      else if (cached.note === "no_match" || cached.note === "no_exact_email_match")
        this.stats.noMatch++;
      else if (cached.note && cached.note.startsWith("ambiguous_")) this.stats.ambiguous++;
      else if (cached.note && cached.note.startsWith("lookup_error")) this.stats.errors++;
      return { ...cached, source: "cache" };
    ...
    let matches;
    try {
      matches = await this.cloudClient.searchUsers(trimmedEmail, 50);
    } catch (e) {
      const result = {
        accountId: null,
        source: null,
    ...
    const atlassianUsers = (matches || []).filter(
      (m) => (m.accountType || "atlassian") === "atlassian",
    );

    const exactMatches = atlassianUsers.filter(
      (m) => (m.emailAddress || "").toLowerCase() === lowerEmail,
    );
    ...
    if (exactMatches.length === 1) {
      ...
      result = {
        ...
        emailAddress: m.emailAddress || trimmedEmail,
      };
      this.stats.resolved++;
    } else if (exactMatches.length === 0) {
      // The Cloud search returned no candidate with this exact email — either
      // the privacy settings hide the email, or no Cloud user has it. Either
      // way we refuse to guess.
      result = {
        accountId: null,
        source: null,
        note: atlassianUsers.length === 0 ? "no_match" : "no_exact_email_match",
        candidateCount: atlassianUsers.length,
        displayName: null,
        emailAddress: trimmedEmail,
      };
      this.stats.noMatch++;
    } else {
      // Multiple Cloud users with the same email — extremely rare but possible
      // for legacy tenants. Refuse to guess.
      result = {
        accountId: null,
        source: null,
        note: `ambiguous_exact_email_${exactMatches.length}`,
        candidateCount: exactMatches.length,
        displayName: null,
        emailAddress: trimmedEmail,
      };
