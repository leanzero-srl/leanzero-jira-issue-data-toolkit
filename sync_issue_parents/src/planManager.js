...
class PlanManager {
  ...
  createMasterIndex(runId, extra = {}, filePrefix = "") {
    ...
    this.masterIndexPath = path.join(this.planDir, `master_${prefix}${runId}.json`);
    this.masterIndex = {
      version: "1.0",
  ...
  findLatestMasterIndex(filePrefix = "") {
    if (!fs.existsSync(this.planDir)) return null;

    const startsWith = filePrefix ? `master_${filePrefix}_` : "master_";
    const files = fs.readdirSync(this.planDir)
      .filter((f) => f.startsWith(startsWith) && f.endsWith(".json"))
      .filter((f) => {
        // When prefix is empty, exclude prefixed files (e.g. master_reporter_*).
        if (filePrefix) return true;
        const tail = f.slice("master_".length, -".json".length);
        return /^\d+$/.test(tail);
      })
      .sort()
      .reverse();
  ...
  createPlan(runId, issuesMap, filePrefix = "") {
    if (!fs.existsSync(this.planDir)) {
      fs.mkdirSync(this.planDir, { recursive: true });
    }

    const prefix = (filePrefix || this.filePrefix || "");
    const prefixSeg = prefix ? `${prefix}_` : "";
    const planFile = path.join(this.planDir, `plan_${prefixSeg}${runId}.json`);
