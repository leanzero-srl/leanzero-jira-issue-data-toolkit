...
class DatacenterClient {
  ...
  /**
   * Fetch a DC issue's issue-to-issue links (the `issuelinks` field).
   *
   * Each raw entry has a `type` ({ id, name, inward, outward }) and EXACTLY ONE
   * of `inwardIssue` / `outwardIssue` (the OTHER endpoint of the link, relative
   * to issueKey). We normalize each into a direction-tagged descriptor so the
   * processor can canonicalize it without re-inspecting the raw shape.
   *
   * Returns:
   *   { key, links: [ { id, typeName, typeInward, typeOutward, direction, otherKey } ] }
   * or on 404/403/other failure:
   *   { key, error: "..." }
   *
   * `direction` is 'outward' when the entry carried an outwardIssue (issueKey
   * {type.outward} otherKey) and 'inward' when it carried an inwardIssue
   * (issueKey {type.inward} otherKey).
   */
  async getIssueLinks(issueKey) {
    const path = `/rest/api/2/issue/${encodeURIComponent(issueKey)}?fields=issuelinks`;
    let res;
    try {
      res = await this.makeRequest("GET", path);
    } catch (error) {
    ...
    const rawLinks = res.fields?.issuelinks || [];
    const links = [];
    for (const l of rawLinks) {
      const type = l.type || {};
      const typeName = type.name || null;
      if (!typeName) continue; // can't recreate a typeless link
      let direction = null;
      let otherKey = null;
      if (l.outwardIssue && l.outwardIssue.key) {
        ...
        continue; // neither endpoint present — nothing to link
      }
      links.push({
        id: l.id || null,
        typeName,
    ...
    return { key: res.key || issueKey, links };
  }
